...
let highlighterLoader

function createCodeWrapper(html) {
...
function trimCodeWhitespace(wrapper) {
...
function stripTwoslashReferenceLine(html) {
...
async function loadHighlighter() {
  ...
  highlighterLoader = (async () => {
    ...
    const twoslashFetch = async (input, init) => {
    ...
    return {
      ...
      twoslashTransformer: createTransformerFactory(
        (code, lang, options) => twoslash.runSync(code, lang, options),
        rendererRich()
      ),
    }
  })()
...
function renderCodeBlock(highlighter, twoslashTransformer, code, lang) {
  const options = {
    lang,
    theme: 'css-variables',
  }
...
export default async function highlight() {
  ...
  codeBlocks.forEach((block) => {
    ...
    const stage = pre?.closest('.stage')

    pre?.replaceWith(wrapper)

    if (stage instanceof HTMLElement && wrapper?.classList.contains('twoslash')) {
      stage.dataset.hasTwoslash = 'true'
    }
  })
}
